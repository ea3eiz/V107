...
echo "Valor actual Indicativo: \33[1;33m${ind#*=}\33[1;37m"
                        ...
                        #Convierte indicativo si se introduce en minúsculas a Mayúsculas
                        tu_indicativo=`echo "$tu_indicativo" | tr [:lower:] [:upper:]`
                        sudo sed -i "117c dplusLogin=$tu_indicativo  " /usr/local/etc/opendv/ircddbgateway
...
echo "Valor actual del RFModeHang = : \33[1;33m${modehang1#*=}\33[1;37m"
                      read -p 'Introduce el valor para RFModeHang (optimo=5): ' rfmodehang
                          letra=c
                          if [ $largo = 2 ]
                          then
...
do
            ...
            #MMDVMNXDN.ini
            sed -i "90c Module=$module" /home/pi/MMDVMHost/MMDVMNXDN.ini
